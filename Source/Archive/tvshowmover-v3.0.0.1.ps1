...
function Sort-TVShows
{
	...
	# Check All Folders for illegal characters in name and rename them.
	
	foreach ($folder in (Get-ChildItem -Path $sourceFolder -Recurse))
	{
		$folder | Rename-Item -NewName { $_.Name -replace '\[|\]|\#|\{|\}|\!|\@|\$|\%|\^|\&|\*|\(|\)', "" } -ErrorAction SilentlyContinue
	}
	...
	{
		$cleanedfile = $file.FullName -replace '\[|\]|\#|\{|\}|\!|\@|\$|\%|\^|\&|\*|\(|\)', ""
		...
		if ($parsed -ne $null)
		{
			...
			if ($showPath -ne $null)
			{
				...
				Move-Item -Path $cleanedfile -Destination $destinationPath -Force
